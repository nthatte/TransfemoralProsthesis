...
int main(void) {
	...
	switch (medulla_id & MEDULLA_ID_PREFIX_MASK) {
		...
		case MEDULLA_KNEE_ID_PREFIX:
			...
			#endif
			initialize = knee_initialize;
			enable_outputs = knee_enable_outputs;
			disable_outputs = knee_disable_outputs;
			update_inputs = knee_update_inputs;
			run_halt = knee_run_halt;
			update_outputs = knee_update_outputs;
			estop = knee_estop;
			check_error = knee_check_error;
